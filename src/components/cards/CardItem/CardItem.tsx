...
//   card: DbCard;
// }
// 1. Добавляем variant в интерфейс
interface CardItemProps {
  card: DbCard;
  variant?: "default" | "glass"; // 👈 Новый пропс
}


const CardItem = ({ card, variant = "default" }: CardItemProps) => {
  const frontImage = card.faces?.[0]?.images?.normal || "";
  const backImage = card.faces?.[1]?.images?.normal;

  const cardClassName = `${css.itemCard} ${variant === "glass" ? css.variantGlass : css.variantDefault}`;

  return (
    <div className={cardClassName}>
      
      {/* 🔹 Оборачиваем картинку в relative div, чтобы спозиционировать сердечко поверх нее */}
      ...
      <div className={css.bottomSection}>
        ...
        <div className="flex items-center gap-2 w-full mt-2">
          <AddToCartSection 
            ...
            // Кнопка корзины займет всё свободное место (flex-1)
